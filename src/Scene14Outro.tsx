...
export const Scene14Outro: React.FC = () => {
	...
	useEffect(() => {
		...
		for (let i = -100; i < 1200; i++) {
			...
			line.setAttribute('style', 'stroke:black;stroke-width:6');
			...
			line2.setAttribute(
				'style',
				'stroke:black;stroke-width:6;stroke-dasharray: 2 96'
			);
	...
	const clipLeading = interpolate(frame, [470, 800], [100, 0], {
		extrapolateLeft: 'clamp',
		extrapolateRight: 'clamp',
	});

	const clipTrailing = interpolate(frame, [485, 815], [0, 100], {
		extrapolateLeft: 'clamp',
		extrapolateRight: 'clamp',
	});
		...
		<AbsoluteFill style={{transform: `translateX(${slideIn}%)`}}>
			<div
				style={{
					position: 'relative',
					zIndex: 20,
					clipPath: `inset(0 ${clipLeading}% 0 ${clipTrailing}%)`,
					transform: 'translateY(600px)',
				}}
			>
				<svg
					ref={sineWave}
					style={{overflow: 'visible', transform: `translate(-165px)`}}
				></svg>
				<svg
					ref={flippedWave}
					style={{overflow: 'visible', transform: `translate(-275px)`}}
				></svg>
			</div>
			<AbsoluteFill>
				<div
